...
export default function TestListS() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8080/api/tests/userKey/${hashedKey}/categoryId/1`);
    ...
    sx={{
      ...
      <List>
        {items
          ...
          .map(item => (
            <ListItem key={item.id} sx={{ background: '#40393A', marginBottom: '5px', width:'100%' }} secondaryAction={
