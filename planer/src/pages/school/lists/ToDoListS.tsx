...
export default function ToDoListS() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8080/api/tasks/userKey/${hashedKey}/categoryId/1/subCategoryId/1`);
  ...
  return (
    <Box sx={{ flexGrow: 1, maxWidth: 752, background: '#D7B3B3' }}>
      <FormControlLabel
        control={
          <StyledCheckbox
        ...
        label="Hide Finished To-Do tasks"
      />
      <IconButton edge="end" aria-label="delete" onClick={() => navigate('/newObject/NewToDo')}>
        <AddIcon />
      </IconButton>
      <List>
        {items
